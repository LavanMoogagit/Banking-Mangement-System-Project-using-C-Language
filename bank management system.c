...

{
   ...
   account_no_regenerate:
   check.acc_no=(((add.deposit.year*100)+(add.deposit.day))*10000)+(rand() % (9999 - 0 + 1) + 0);
         while(fscanf(ptr,"%d %s %d/%d/%d %d %s %s %lf %s %f %d/%d/%d\n",&add.acc_no,add.name,&add.dob.month,&add.dob.day,&add.dob.year,&add.age,add.address,add.citizenship,&add.phone,add.acc_type,&add.amt,&add.deposit.month,&add.deposit.day,&add.deposit.year)!=EOF)
    {
        if (check.acc_no==add.acc_no)
            {//printf("Account no. already in use!");
            fordelay(1000000000);
                goto account_no_regenerate;
...
void close(void)
{
    printf("\n\n\n\nThis C Mini Project is developed by GANESH POLADASRAI (160119737158) AND G NIKHIL (160119737164)!");
    }
